...
export function MapContainer() {
  ...
  const { buildings, entrances, isLoading } = useMapData(bbox, viewState.zoom);
  ...
  const layers = [
    new GeoJsonLayer({
      ...
      updateTriggers: { getFillColor: [scoreRange] },
    }),
    showEntrances && new ScatterplotLayer({
      id: "entrances",
  ...
  return (
    <div ref={containerRef} className="relative w-full h-full">
      <DeckGL viewState={viewState} onViewStateChange={onViewStateChange} controller={true} layers={layers}>
        <Map mapboxAccessToken={process.env.NEXT_PUBLIC_MAPBOX_TOKEN} mapStyle="mapbox://styles/mapbox/dark-v11" />
      </DeckGL>

      <MapControls
        showHeatmap={showHeatmap} showEntrances={showEntrances} scoreRange={scoreRange}
        onToggleHeatmap={() => setShowHeatmap(!showHeatmap)}
        onToggleEntrances={() => setShowEntrances(!showEntrances)}
        onScoreRangeChange={setScoreRange}
        isLoading={isLoading} zoom={viewState.zoom}
      />

      {selectedBuildingId && (
        <EvidencePanel buildingId={selectedBuildingId} onClose={() => setSelectedBuildingId(null)} />
      )}
    </div>
  );
}
