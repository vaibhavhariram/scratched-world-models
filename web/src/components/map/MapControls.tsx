    ...
    <div className="absolute top-4 left-4 bg-gray-900/90 text-white p-4 rounded-lg shadow-lg space-y-3 w-64 backdrop-blur-sm">
      ...
      <div className="space-y-2">
        <label className="flex items-center gap-2 text-sm cursor-pointer">
          ...
          />
          Difficulty Heatmap
          <span className="text-xs text-gray-500">(zoom &lt; 15)</span>
        </label>
        ...
        <label className="flex items-center gap-2 text-sm cursor-pointer">
          <input
            ...
            onChange={onToggleEntrances}
            className="rounded"
          />
          Entrance Points
          <span className="text-xs text-gray-500">(zoom &ge; 16)</span>
        </label>
      </div>

      <div className="space-y-1">
        <label className="text-xs text-gray-400">
          Difficulty Filter: {scoreRange[0].toFixed(1)} -{" "}
        ...
        <input
          ...
          value={scoreRange[1]}
          onChange={(e) =>
            onScoreRangeChange([scoreRange[0], parseFloat(e.target.value)])
          }
          className="w-full"
        />
      </div>
